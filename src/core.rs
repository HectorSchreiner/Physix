...
pub mod physix {
    ...
    impl Vector3 {
        ...
        pub fn mul_assign(&mut self, value: real) {
            self.x *= value;
            self.y *= value;
            self.z *= value;
        }
...
pub fn rk4(f: Vector3, timestep: physix::real) -> Vector3 {
    let k1 = f;
    let k2 = f + k1.mul(timestep * 0.5);
    let k3 = f + k1.mul(timestep * 0.5);
    let k4 = f + k3.mul(timestep);

    f + (k1 + k2.mul(2.0) + k3.mul(2.0) + k4).mul(timestep / 6.0)
}
